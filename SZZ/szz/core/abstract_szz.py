...
class AbstractSZZ(ABC):
        ...
        if not use_temp_dir:
            ...
            if repos_dir:
                ...
                if os.path.isdir(repo_dir):
                    self._repository = Repo(repo_dir)
                else:
                    print("Clone repository", repo_url)
                    Repo.clone_from(url=repo_url, to_path=repo_dir)
                    if os.path.isdir(repo_dir):
                        self._repository = Repo(repo_dir)
                    else:
            ...
            self._repository_path = os.path.join(
                self.__temp_dir, repo_full_name.replace("/", "_")
            )
                ...
                if repos_dir:
                    ...
                    if os.path.isdir(repo_dir):
                        ...
                        if os.path.isdir(repo_dir):
                            ...
                            log.error(
                                f"unable to find local repository path: {repo_dir}"
                            )
                            exit(-4)
                else:
    ...
    @abstractmethod
    def find_bic(
        self, fix_commit_hash: str, impacted_files: List["ImpactedFile"], **kwargs
    ) -> Set[Commit]:
        """
    ...
    def get_impacted_files(
        ...
        for mod in fix_commit.modifications:
            ...
            if (
                ...
                or mod.change_type == ModificationType.RENAME
            ):
    ...
    def _blame(
        ...
        for entry in self.repository.blame_incremental(
            **kwargs, rev=rev, L=mod_line_ranges, file=file_path
        ):

            for line_num in entry.orig_linenos:  #
                source_file_content = self.repository.git.show(
                    f"{entry.commit.hexsha}:{entry.orig_path}"
                )
                ...
                for mod in blame_commit.modifications:
                    if mod.new_path == file_path:
                        ...
                        # Adjusting the approach to include "@" back to the strings where it was removed, except for the first element
                        hunks_adjusted = [hunks[0]] + ["@" + s for s in hunks[1:]]
                        for hunk in hunks_adjusted:
                            line_number = line_num
                            lines = hunk.split("\n")
                            start_line = int(lines[0].split(" ")[2].split(",")[0])
                            hunk_lines = int(lines[0].split(" ")[2].split(",")[1])

                            old_start = int(lines[0].split(" ")[1].split(",")[0])
                            old_lines = int(lines[0].split(" ")[1].split(",")[1])
                ...
                end_line = min(len(lines) - 1, line_num + context_lines)
                for i in range(start_line, end_line + 1):
                    context_around_line += lines[i] + "\n"
                b_data = BlameData(
                    entry.commit,
                    line_num,
                    line_str,
                ...
                if skip_comments and self._is_comment(
                    line_num, source_file_content, ntpath.basename(b_data.file_path)
                ):
    ...
    def _is_comment(
        self, line_num: int, source_file_content: str, source_file_name: str
    ) -> bool:
        """
        ...
        comment_ranges = parse_comments(
            source_file_content, source_file_name, self.__temp_dir
        )
    ...
    def get_commit_diff(self, commit: Commit):
        return self.repository.diff(commit.parent[0], commit)

    def get_commit_unified_diff_text(self, hexsha: str):
        try:
            return self.repository.git.diff(
                hexsha + "~1", hexsha, ignore_blank_lines=True, ignore_space_at_eol=True
            )
        except Exception as e:
            print("may be the init commit. Error message: " + str(e))
            return None
...
class BlameData:
    ...
    def __init__(
        ...
        self.file_path = file_path
        self.context_around_line = context_around_line
        self.hunk = hunk
