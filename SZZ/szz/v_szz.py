...
from log_generation import GitLog  # 从log_generation.py中导入GitLog类
from setting import WORK_DIR, DATA_FOLDER  # 从setting.py中导入WORK_DIR和DATA_FOLDER变量
import Levenshtein
                ...
                for entry in blame_data:
                    ...
                    while True:
                        if imp_file.file_path.endswith(".java"):
                            ...
                            previous_commits.append({"commit":blame_result.commit.hexsha, "line number": blame_result.line_num,"line content":blame_result.line_str,"change type":change_type,})
                    ...
                    bug_introd_commits.append({'line_num':entry.line_num, 'line_str': entry.line_str, 'file_path': entry.file_path,"content around":entry.context_around_line, 'previous_commits': previous_commits})
    ...
    def map_modified_line_java(self, blame_entry, blame_file_path):
        ...
        if os.path.exists(mapping_db_file):
            ...
            if commit_id not in mapping_db:
                ...
                subprocess.call(mapping_cmd.format(project=self.repo_full_name, commit_id=commit_id, output=os.path.join(ast_map_temp, "tmp.json"), file_path=file_path), cwd=self.ast_map_path, shell=True)
                # 将输出显示到终端
    ...
    def map_modified_line(self, blame_entry, blame_file_path):
        ...
        for mod in blame_commit.modifications:
            ...
            if blame_entry.line_str:  #旧删除行
                ...
                # 首先按照相似度降序排列，如果相似度相同则按照行号差值升序排列。
